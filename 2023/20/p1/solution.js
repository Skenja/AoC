...
const countNumberOfPulsesSent = (modules) => {
    let countLow = 1; // 1 for the button press
    let countHigh = 0;
    ...
    while (queue.length > 0) {
        ...
        const addToQueue = module.outputs.map(x => [x, outputPulse]);

        for (const [_, pulse] of addToQueue) {
            if (pulse === 'low') {
                countLow++;
            } else {
                countHigh++;
...
const warmUpLines = (modules) => {
    const result = [0, 0];

    for (let i = 0; i < 1000; i++) {
        const [low, high] = countNumberOfPulsesSent(modules);
...
const solve = async (fileName) => {
    ...
    const count = warmUpLines(modules);
